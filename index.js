...
app.post("/update-cobj", async (req, res) => {
  ...
  const species = Array.isArray(req.body.species) ? req.body.species[0] : req.body.species;
...
app.get("/", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://api.hubapi.com/crm/v3/objects/${CUSTOM_OBJECT}?properties=name,species,age`,
      {
        headers: {
          Authorization: `Bearer ${HUBSPOT_TOKEN}`,
        },
